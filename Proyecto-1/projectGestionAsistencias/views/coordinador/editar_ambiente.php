...
$ambiente = $controller->obtenerAmbientePorId($id); // Método que debes implementar en el controlador

// Verificar si el ambiente existe
if (!$ambiente) {
    echo "Error: Ambiente no encontrado.";
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($controller->editarAmbiente($id, $nombre)) {
        header('Location: ambientes.php');
        exit();
    } else {
        $error = "Error al editar el ambiente";
    }
}
?>
...
<body>
    ...
    <form method="POST">
        ...
        <input type="text" id="nombre" name="nombre" value="<?php echo htmlspecialchars($ambiente['nombre'], ENT_QUOTES, 'UTF-8'); ?>" required>
        <br>
